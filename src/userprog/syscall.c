 ...
 */
static struct fd_entry *create_fd(struct file *file, struct dir *dir, bool is_directory) {
    ...
    fd_entry->f = file;
    fd_entry->is_directory = is_directory;
    fd_entry->dir = dir;
...
static void syscall_handler (struct intr_frame *f UNUSED) {
    ...
    switch (syscall_num) {
        ...
        case SYS_CREATE: {
            ...
            // create the file and return the result 
        ...
        case SYS_OPEN: {
            ...
            // open the file and create a file descriptor entry
            ...
            // check if inode is a directory or file
            bool is_directory = inode_is_directory(inode);
            struct fd_entry *fd = NULL;
            if (is_directory) {
                // open as directory
                struct dir *dir = dir_open(inode);
                if(dir != NULL) {
                    fd = create_fd(NULL, dir, true);
                    if (fd == NULL) {
                        dir_close(dir);
                    }
                } else {
            ...
            palloc_free_page(fileName);
            // check if file descriptor entry was created successfully
            if (fd == NULL) {
        ...
        // Case 8: for the FILESIZE system call
        case SYS_FILESIZE: {
        ...
        case SYS_CLOSE: {
            ...
            // close the file/directory and remove the fd entry 
            if (fd_entry->is_directory) {
        ...
        case SYS_CHDIR: {
            // get the argument: directory name
            const char *dirNamePtr;
            if (!copy_data(&dirNamePtr, sp + 4, sizeof(const char *))) {
                system_exit(-1);
            }
            if (dirNamePtr == NULL) {
                system_exit(-1);
            }
            // copy the directory name string from user to kernel space
            char *dirName = copy_string((char *)dirNamePtr);
            if (dirName == NULL) {
                system_exit(-1);
            }
        ...
        // Case 15: for the MKDIR system call
        case SYS_MKDIR: {
            // get the argument: directory name
            const char *dirNamePtr;
            if (!copy_data(&dirNamePtr, sp + 4, sizeof(const char *))) {
        ...
        // Case 16: for the READDIR system call
        case SYS_READDIR: {
            // get the arguments: fd, name
            int fd;
            char *name;
            if (!copy_data(&fd, sp + 4, sizeof(int))) {
                system_exit(-1);
            }
            if (!copy_data(&name, sp + 8, sizeof(char *))) {
                system_exit(-1);
            }
            // ensure name pointer is valid
            if (name == NULL) {
                system_exit(-1);
            }
            // find the file descriptor entry
            struct fd_entry *fd_entry = find_fd(fd);
            if (fd_entry == NULL || !fd_entry->is_directory) {
                f->eax = false;
                break;
            }
            // create a buffer to hold the read name
